...
const OrderCreate = () => {
	const { products, profile } = useLoaderData();
	...
	const addOrderItem = (e) => {
		...
		const totalAmount =
			Number(selectedProduct.pricePerBag) * Number(noOfBags);
		...
		const existingOrderItemIndex = orderList.findIndex(
			(orderItem) => orderItem.productRefId === selectedProduct.refId
		);
		...
		if (existingOrderItemIndex !== -1) {
			...
			setOrderList((prev) => [newOrderItem, ...prev]);
		}
	...
	const updateOrderItem = () => {
		...
		const updatedOrderList = orderList.map((orderItem) =>
			orderItem === selectedOrderItem ? updatedOrderItem : orderItem
		);
		...
		<Stack borderLeftWidth='1px' p='4'>
			...
			<Stack bg='gray.200' p='4' spacing='3'>
				<HStack justifyContent='space-between'>
					...
					<Text fontWeight='bold'>{totalNoOfBags}</Text>
				</HStack>
				<HStack justifyContent='space-between'>
...
const ShippingForm = ({
	...
	return (
		...
		>
			...
			<Flex
				gap={{ base: '4', md: '6' }}
				direction={{ base: 'column', sm: user ? 'column' : 'row' }}
			>
				<FormControl>
...
const OrderItem = ({
	...
	products,
	deleteOrderItem,
	showUpdateOrderItemForm,
}) => {
	const product = products.filter(
		(prod) => prod.refId === orderItem.productRefId
	)[0];
				...
				<HStack justifyContent='space-between'>
					...
					<HStack>
						<IconButton
							...
							colorScheme='purple'
							size='xs'
							onClick={() => showUpdateOrderItemForm(orderItem)}
						/>
						<IconButton
							icon={<FaRegTrashCan />}
							colorScheme='red'
			...
			<CardBody px='3' py='2'>
				<SimpleGrid columns={{ base: 1, md: 2 }} spacing='2'>
					...
					<Box>
						...
						<Text fontSize='sm'>{orderItem.noOfBags}</Text>
					</Box>
					<Box>
						<Heading
							fontSize='sm'
					...
					<Box>
						...
						<Text fontSize='sm'>{orderItem.totalWeight}</Text>
					</Box>
				</SimpleGrid>
			</CardBody>
		</Card>
